...
// //numtasks number of procesess
// //numtasks id of process
// MPI_Comm_size(MPI_COMM_WORLD,&numtasks);
// MPI_Comm_rank(MPI_COMM_WORLD,&taskid);
// int counter= 0;
// if(taskid%2){
//   while(counter<PROS){
//     MPI_Send(buffer , n*d , MPI_DOUBLE, (taskid + 1)%PROS , 0 , MPI_COMM_WORLD );
//     result = kNN(X,buffer , n ,n , k ,d );
//     Mpi_Recv(buffer , n*d , MPI_DOUBLE, taskid - 1 , 0 , MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//     for(int l=((counter+1)*n*k; l<((counter+2)*n*k); l++){
//       for(int j=0; j<d; j++){
...
//     MPI_Send(buffer , n*d , MPI_DOUBLE, (taskid + 1)%PROS , 0 , MPI_COMM_WORLD );
//     result = kNN(X,buffer , n ,n , k ,d );
//     Mpi_Recv(buffer , n*d , MPI_DOUBLE, (PROS+taskid- 1)%PROS , 0 , MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//     for(int l=((counter+1)*n*k; l<((counter+2)*n*k); l++){
//       for(int j=0; j<d; j++){
//         *(dist + l*d + j) = *(result.ndist + l*d + j);
//         *(idx + l*d + j) =  *(result.nidx + l*d + j) ;
